...
class GraphRAGCore:
    ...
    async def insert(self, corpus: List[Dict[str, Any]]) -> None:
        ...
        load_existing = not force and not add

        if load_existing:
            graph_path = os.path.join(self.base_dir, f"{self.config.index_name}.pkl")
            logger.info("📂 LOAD EXISTING MODE: skipping corpus processing and summary generation")

            if corpus:
                logger.info(f"Ignoring {len(corpus)} input documents because load existing mode is enabled")
