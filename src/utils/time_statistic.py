...
class GraphBuildingTimeStatistic(TimeStatistic):
    ...
    def get_graph_building_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entity_extraction_percentage": round((self.entity_extraction_time / total_time * 100),
                                                  1) if total_time > 0 else 0,
            "relationship_extraction_percentage": round((self.relationship_extraction_time / total_time * 100),
                                                        1) if total_time > 0 else 0,
            "graph_construction_percentage": round((self.graph_construction_time / total_time * 100),
                                                   1) if total_time > 0 else 0,
            "summary_generation_percentage": round((self.summary_generation_time / total_time * 100),
                                                   1) if total_time > 0 else 0
        }
...
class QueryTimeStatistic(TimeStatistic):
    ...
    def get_query_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "answer_generation_percentage": round((self.answer_generation_time / total_time * 100),
                                                  1) if total_time > 0 else 0,

            "retrieval_step_percentages": {
                "entity_extraction": round((self.entity_extraction_time / self.retrieval_time * 100),
                                           1) if self.retrieval_time > 0 else 0,
                "similar_entity_search": round((self.similar_entity_search_time / self.retrieval_time * 100),
                                               1) if self.retrieval_time > 0 else 0,
                "triple_retrieval": round((self.triple_retrieval_time / self.retrieval_time * 100),
                                          1) if self.retrieval_time > 0 else 0,
                "summary_retrieval": round((self.summary_retrieval_time / self.retrieval_time * 100),
                                           1) if self.retrieval_time > 0 else 0,
                "triple_reranking": round((self.triple_reranking_time / self.retrieval_time * 100),
                                          1) if self.retrieval_time > 0 else 0,
                "chunk_retrieval": round((self.chunk_retrieval_time / self.retrieval_time * 100),
                                         1) if self.retrieval_time > 0 else 0,
                "prompt_generation": round((self.prompt_generation_time / self.retrieval_time * 100),
                                           1) if self.retrieval_time > 0 else 0
            }
        }
