...
class FinalReportGenerator:
    ...
    def _generate_query_processing_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_breakdown': {
                ...
                'average_answer_generation_time': round(total_answer_generation_time / total_queries,
                                                        2) if total_queries > 0 else 0
            },
            'cost_breakdown': {
                ...
                'average_tokens_per_query': round(
                    (total_retrieval_tokens + total_answer_generation_tokens) / total_queries,
                    0) if total_queries > 0 else 0,
                'average_cost_per_query': round(total_query_cost / total_queries, 4) if total_queries > 0 else 0
            },
            'efficiency_metrics': {
                'tokens_per_second': round(
    ...
    def _generate_performance_analysis(self) -> Dict[str, Any]:
        ...
        graph_time = self.graph_building_stats['time'].get('total_graph_building_time',
                                                           0) if self.graph_building_stats else 0
    ...
    def _generate_recommendations(self) -> Dict[str, Any]:
        ...
        if self.query_stats:
            avg_query_time = sum(stat['time'].get('total_query_time', 0) for stat in self.query_stats) / len(
                self.query_stats)
