...
from src.prompt.query_prompt import SUMMARY_QUERY_PROMPT
...
class SummaryRetriever:
    ...
    async def build_summary_embeddings(self) -> None:
        if not self.summaries:
            logger.error("No session summaries loaded")
            return
    ...
    async def retrieve_relevant_summaries(self, question: str, top_k: int = None) -> List[Dict[str, Any]]:
        ...
        if not self.summaries or self.summary_embeddings is None:
            logger.warning("No summaries or embeddings loaded")
            return []
        ...
        for entity_idx, entity_embedding in enumerate(entity_embeddings):
            for summary_idx, summary_embedding in enumerate(self.summary_embeddings):
                ...
                summary_scores[summary_idx] = max(summary_scores[summary_idx], similarity)
    ...
    def get_session_ids_from_summary(self, summaries: List[Dict[str, Any]]) -> List[str]:
        session_ids = []
    ...
    def create_summary_query_prompt(self, question: str, summaries: List[Dict[str, Any]], triples: str,
                                    chunks: str) -> str:
