...
from src.prompt.query_prompt import QUERY_PROMPT, SUMMARY_QUERY_PROMPT
from src.utils.time_statistic import QueryTimeStatistic
from src.utils.cost_manager import QueryCostManager
...
class QueryProcessor:
    ...
    def _extract_and_format_timestamp(edge_data: Dict[str, Any]) -> str:
        ...
        if not timestamp and "session_times" in edge_data:
            ...
            if valid_times:
                timestamp = sorted(valid_times, reverse=True)[0]
    ...
    async def initialize_summary_data(self) -> bool:
        if not self.summary_retriever:
            logger.warning("Summary retriever not initialized")
            return False
    ...
    async def _process_unified_query(self, question: str, question_time: str = "") -> Dict[str, Any]:
        ...
        answer = await self._generate_answer(formatted_prompt)
    ...
    async def _extract_query_entities(self, question: str, question_time: str = "") -> List[str]:
        if not question:
            return []
        try:
            entities_data = await self.llm.extract_entities(question, question_time)
    ...
    async def _generate_answer(self, formatted_prompt: str) -> str:
        try:
    ...
    async def _find_similar_entities(self, question: str, query_entities_with_types: List[Dict[str, str]] = None) -> List[Dict[str, Any]]:
        ...
        if not self.embedding_manager:
            logger.warning("Embedding manager not initialized, falling back to string matching")
        ...
        if hasattr(self.dynamic_memory, "graph_builder") and self.dynamic_memory.graph_builder.graph:
            ...
            for node, data in graph.nodes(data=True):
                if "entity_type" in data or "entity" in data:
                    all_entities.append({
                        "name": node,
            ...
            for entity_name in self.dynamic_memory.entity_name_to_index:
                all_entities.append({
                    "name": entity_name,
                    "type": "unknown",
                    "description": ""
        ...
        try:
            ...
            for i, entity in enumerate(all_entities):
                ...
                if graph and entity_name in graph:
                    ...
                    if "embedding" in node_data:
                        ...
                        texts_to_compute.append(entity_name)
            ...
            if texts_to_compute:
                ...
                if computed_embeddings and len(computed_embeddings) == len(texts_to_compute):
                    for idx, computed_embedding in zip(indices_to_compute, computed_embeddings):
                        entity_embeddings[idx] = computed_embedding
    ...
    async def _find_similar_entities_fallback(self, query_entities: List[str]) -> List[Dict[str, Any]]:
        ...
        for query_entity in query_entities:
            ...
            for graph_entity in graph_entities:
                if query_entity_lower in graph_entity.lower() or graph_entity.lower() in query_entity_lower:
                    if graph_entity not in [e["name"] for e in relevant_entities]:
                        relevant_entities.append({
                            "name": graph_entity,
                            "type": "unknown",
    ...
    async def _get_relevant_triples(self, question: str, entities: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for src, tgt, data in graph.edges(data=True):
            if src in entity_ranking or tgt in entity_ranking:
                ...
                src_ranking_bonus = 1.0 / (entity_ranking.get(src, len(entities)) + 1)
                tgt_ranking_bonus = 1.0 / (entity_ranking.get(tgt, len(entities)) + 1)
                entity_bonus = max(src_ranking_bonus, tgt_ranking_bonus)
    ...
    async def _get_chunk_content(self, chunk_id: str) -> str:
        ...
        enable_sessiontime = getattr(self.config.retriever, "enable_sessiontime", False)
        ...
        if not user_utterances:
            ...
            for sentence in sentences:
                ...
                if not any(keyword in sentence.lower() for keyword in
                           ['assistant:', 'i can help', 'here are', 'let me', 'you should']):
                    if len(sentence) > 10:
