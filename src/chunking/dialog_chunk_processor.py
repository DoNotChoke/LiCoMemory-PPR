...
class DialogChunkProcessor:
    def __init__(self, data_type: str = "LongmemEval"):
        self.data_type = data_type
        logger.info(f"Dialog Chunk Processor initialized for {data_type} dataset")

    def parse_dialog_turns(self, context_text: str) -> List[Dict[str, Any]]:
        dialog_turns = []
    ...
    def create_dialog_chunks(self, session_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if not dialog_turns:
            logger.warning(f"No dialog turns for session {session_id}")
            return []
